...
- `explain_attribute( target, name )` — returns the complete decision
  ...
  behaviors per evaluated level, and per-operation verdicts with the
  rule(s) that decided each outcome.
- `AttributeExplanation`, `AssignVerdict`, `DeleteVerdict`,
  `SurveyVerdict`, `DecisionRule` — immutable (frozen, concealed)
  records, with all nested collections genuinely immutable.
- The decision cascade in `behaviors.py` is extracted into pure helpers
  consumed by both the behavior cores and the explanations, so that the
  explanation is the behavior, not a reimplementation of it.
...
- New module `sources/classcore/standard/explanations.py`; nomina aliases
  for the records.
